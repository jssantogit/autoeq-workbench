...
describe('Squiglink source parity styles', () => {
  it('keeps the toolbar theme toggle in normal horizontal scroll flow', () => {
    const themeToggleRule = rule(baseCss, '.graph-toolbar > .theme-toggle')
  ...
  it('uses solid accent fill for selected dock and graph controls', () => {
    const dockSelected = rule(indexCss, ".dock-tabs button[aria-selected='true']")
    ...
    const graphSelected = rule(themeCss, ".graph-toolbar button[aria-pressed='true']")
