...
export function createSquiglinkGraph(
  ...
  function renderLabels(palette: SquiglinkGraphPalette): void {
    ...
    groups.each(function renderLabel(item, index) {
      ...
      group.selectAll<SVGLineElement, PreparedSeries>('line').data(item.dashed ? [item] : []).join('line')
        .attr('data-target-label-sample', item.id)
        .attr('x1', 67).attr('x2', 97).attr('y1', labelY - 3).attr('y2', labelY - 3)
        .attr('stroke', item.color).attr('stroke-width', item.lineWidth ?? 1.1)
        .attr('stroke-dasharray', '7 5').attr('stroke-opacity', item.opacity ?? 0.82)
      group.selectAll<SVGTextElement, PreparedSeries>('text').data([item]).join('text')
        ...
        .attr('x', item.dashed ? 102 : 67).attr('y', labelY).attr('fill', item.color)
        .attr('font-size', compact ? 19 : 10).attr('font-weight', 650).text(item.name)
    })
