...
describe('CurveManagerRow', () => {
  ...
  it('uses source-shaped direct cells with a dominant name and no visible kind badge', () => {
    ...
    expect(screen.getByLabelText(`${source.name} offset dB`)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: `Set ${source.name} graph baseline` })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: `Hide ${source.name}` })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: `Remove ${source.name}` })).toBeInTheDocument()
