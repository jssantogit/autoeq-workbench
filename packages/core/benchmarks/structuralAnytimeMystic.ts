...
function runCase(caseId: ManualRegressionCaseId) {
...
const rows = loadManualRegressionCases().map((regressionCase) =>
  runCase(regressionCase.id)
)

console.log(JSON.stringify({
  maxFilters: MAX_FILTERS,
  checkpointsSeconds: CHECKPOINTS_MS.map((value) => value / 1_000),
  rows,
  summary: {
    strictImprovementCases: rows.filter(
      (row) => row.strictImprovementAcrossCheckpoints,
    ).length,
    nonWorseningCases: rows.filter(
      (row) => row.nonWorseningAcrossCheckpoints,
    ).length,
