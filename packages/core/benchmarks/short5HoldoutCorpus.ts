...
for (const benchmarkCase of V2_HOLDOUT_CASES) {
  ...
  const v2 = runStandardAutoEqV2({ ...benchmarkCase, settings: { ...benchmarkCase.settings, timeLimitSeconds: 5 } })
...
const outputDir = resolve(process.env.AUDIT_OUTPUT_DIR ?? 'short5-holdout-corpus')
