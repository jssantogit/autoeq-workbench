...
export function loadCampaignInputs() {
  ...
  const mpStormRaw = read(resolveCapacityRecoveryPath('mp-seeds/titan-to-storm-teacher-student.json'))
  const sameRunRaw = read(resolveCapacityRecoveryPath('same-runtime-tournament/tournament-report.json'))
