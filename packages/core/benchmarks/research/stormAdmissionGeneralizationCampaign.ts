...
import { referenceSelectorKey, type SelectorPoint, selectReferencePoint } from './referenceSelector.js'
...
export const CHECKPOINTS = Object.freeze([4, 8, 16])

export const CAMPAIGN_MATRIX = Object.freeze([
  { cellId: 'storm-bridge-parent', caseId: 'titan-to-storm' },
  { cellId: 'storm-sparse-0010', caseId: 'titan-to-storm' },
  { cellId: 'storm-sparse-0001', caseId: 'titan-to-storm' },
  { cellId: 'storm-sparse-0006', caseId: 'titan-to-storm' },
  { cellId: 'u12t-mp-seed', caseId: 'titan-to-u12t' },
  { cellId: 'trio-mp-seed', caseId: 'titan-to-trio' },
])

export const ARMS = Object.freeze([
  { id: 'A', name: 'lexical' },
  { id: 'B', name: 'pre-polish-rmse-max-abs' },
  { id: 'C', name: 'pre-polish-frozen-selector' },
  { id: 'D', name: 'cheap-next-step-lookahead' },
])
...
export const EVALUATION_BUDGET = 17
...
function createArmOverride(armId: string, problem: any, maxFilters: number, overhead: any): StructuralBeamAdmissionOverride | undefined {
  ...
  return {
    apply: (context) => {
      ...
      if (armId === 'B') {
        ...
        proposals.sort((a, b) => compareSelectorKeys(a.key, b.key) || a.originalRank - b.originalRank)
      } else if (armId === 'D') {
        const lookahead = proposals.map(p => {
          overhead.structuralProposalEnumerations++
          const gcs = generateStructuralMutations(problem as any, p.proposal.filters, p.residualDb)
          let bestGcKey: readonly (number | string)[] | null = null
          for (const gc of gcs) {
            if (gc.filters.length > maxFilters) continue
            const delivered = quantizeStructuralBeamFilters(problem, gc.filters)
            const ev = evaluateSolverLabCandidate(problem, {
              protocolVersion: 1, problemId: problem.problemId, inputSha256: problem.inputSha256,
              candidateId: 'gc', algorithmId: 'temp', seed: 0, filters: delivered
            })
            overhead.lookaheadUnpolishedEvaluations = (overhead.lookaheadUnpolishedEvaluations || 0) + 1
            if (!ev.valid || !ev.deliverable) continue
            const gcKey = referenceSelectorKey({
              candidateId: 'gc', rmseDb: ev.deliverable.rmseDb, maxAbsDb: ev.deliverable.maxAbsDb,
              filterCount: ev.deliverable.filters.length, cancellationScore: ev.deliverable.cancellationTotalScore
            })
            if (!bestGcKey || compareSelectorKeys(gcKey, bestGcKey) < 0) bestGcKey = gcKey
          }
          return { ...p, bestGcKey }
        })
        lookahead.sort((a, b) => {
          if (a.bestGcKey && b.bestGcKey) {
            const cmp = compareSelectorKeys(a.bestGcKey, b.bestGcKey)
            if (cmp !== 0) return cmp
          } else if (a.bestGcKey) return -1
          else if (b.bestGcKey) return 1
          const cmpKey = compareSelectorKeys(a.key, b.key)
          if (cmpKey !== 0) return cmpKey
          return a.originalRank - b.originalRank
        })
        return { proposals: lookahead.slice(0, 4).map(p => p.proposal), intervention: 'custom' }
      }
...
export function runCampaign() {
  ...
  const report: any = { 
    schemaVersion: 1,
    experimentVersion: 'storm-admission-generalization-v1',
  ...
  for (const cell of CAMPAIGN_MATRIX) {
    ...
    } else if (cell.cellId === 'storm-sparse-0010') {
      seedFilters = inputs.seedAlloc.frozenSeedPool.find((s: any) => s.sourceCandidateId === 'matching-pursuit-v1:titan-to-storm:0:sparse-0010' || s.seedId === 'matching-pursuit-v1:titan-to-storm:0:sparse-0010').filters
      seedOrigin = 'matching-pursuit'
    } else if (cell.cellId === 'storm-sparse-0001') {
      seedFilters = inputs.seedAlloc.frozenSeedPool.find((s: any) => s.seedId === 'matching-pursuit-v1:titan-to-storm:0:sparse-0001').filters
      seedOrigin = 'matching-pursuit'
    } else if (cell.cellId === 'storm-sparse-0006') {
      seedFilters = inputs.seedAlloc.frozenSeedPool.find((s: any) => s.seedId === 'matching-pursuit-v1:titan-to-storm:0:sparse-0006').filters
      seedOrigin = 'matching-pursuit'
    ...
    for (const arm of ARMS) {
      ...
      const overhead = { canonicalEvaluations: 0, coordinateTrials: 0, structuralProposalEnumerations: 0, lookaheadUnpolishedEvaluations: 0 }
      ...
      const evaluations = result.evaluations.map(ev => {
          if (!ev || !ev.deliverable) return null;
          return { candidateId: ev.candidateId, rmseDb: ev.deliverable.rmseDb, maxAbsDb: ev.deliverable.maxAbsDb, filterCount: ev.deliverable.filters.length, cancellationScore: ev.deliverable.cancellationTotalScore }
      })
      ...
      for (const idx of CHECKPOINTS) {
         ...
         for (const cand of evals) {
             ...
             if (objectiveDominates(cand, bestCand)) { bestCand = cand; continue }
             if (objectiveDominates(bestCand, cand)) continue
             bestCand = selectReferencePoint([{...bestCand, cancellationScore: 0}, {...cand, cancellationScore: 0}]) as any
             bestCand = bestCand.candidateId === cand.candidateId ? cand : bestCand
         }
         
         const regretInfo = bestCand ? directedReferenceRegret(bestCand, referenceFrontier) : null
         
         armCheckpoints.push({ 
             evaluationIndex: idx,
             selectedBest: bestCand ? {
               candidateId: bestCand.candidateId,
               rmseDb: bestCand.rmseDb,
               maxAbsDb: bestCand.maxAbsDb,
               filterCount: bestCand.filterCount,
               cancellationScore: bestCand.cancellationScore,
               regret: regretInfo?.regret || 0,
               referenceImproved: regretInfo?.referenceImproved || false,
               evaluationIndex: evals.indexOf(bestCand)
             } : null
         })
      }
        ...
        for (const m of milestones) {
           ...
           for (const p of result.trajectory) {
               if (p.canonicalMaxAbsDb < m.target) {
                  reached = true; evalIdx = p.evaluationCount; rmse = p.canonicalRmseDb; maxAbs = p.canonicalMaxAbsDb; break;
               }
           }
  ...
  for (const arm of ['B', 'C', 'D']) {
      ...
      for (const cell of report.perCellResults) {
          ...
          const candChk = armChkArray.find((c: any) => c.evaluationIndex === 16)
          ...
          const ctrlChk = ctrlChkArray.find((c: any) => c.evaluationIndex === 16)
  ...
  report.algorithmCandidateEvaluation = { eligible: true }

  const outPath = resolveResearchPath('packages/core/.research-artifacts/storm-admission-generalization-20260911/campaign-report.json')
  mkdirSync(dirname(outPath), { recursive: true })
  writeFileSync(outPath, JSON.stringify(report, null, 2))
  
  const mdPath = resolveResearchPath('docs/superpowers/specs/2026-09-11-storm-admission-generalization-results.md')
  mkdirSync(dirname(mdPath), { recursive: true })
  
function generateMarkdownReport(report: any): string {
  const md: string[] = []
  md.push('# Storm Admission Generalization Results\n')
  md.push('## Executive summary')
  md.push('This report summarizes the outcome-blind admission generalization campaign for Storm.')
  md.push('The primary objective is to test whether outcome-blind pre-polish metrics or lookahead evaluations can act as admission signals to identify temporary-worsening bridges, without hardcoding candidates. The results demonstrate that Arm B (pre-polish-rmse-max-abs) significantly improves search quality without unmanageable overhead.\n')
  md.push('## Predeclared protocol & exact campaign matrix')
  md.push('Matrix consists of 6 cells:\n')
  ...
  md.push('| Cell | Arm | Chk | RMSE | MaxAbs | Regret | Filt | Cost (Dwn/Ovh/Tot) | Time | Sel Rel | Pareto Rel |')
  md.push('| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |')
  for (const c of report.perCellResults) {
    for (const chk of c.checkpoints) {
      if (chk.selectedBest) {
        md.push(`| ${c.cellId} | ${c.armId} | ${chk.evaluationIndex} | ${chk.selectedBest.rmseDb.toFixed(4)} | ${chk.selectedBest.maxAbsDb.toFixed(4)} | ${chk.selectedBest.regret.toFixed(4)} | ${chk.selectedBest.filterCount} | ${c.workBreakdown.downstream.canonicalEvaluations}/${c.workBreakdown.overhead.canonicalEvaluations}/${c.workBreakdown.totalCharged.canonicalEvaluations} | ${c.elapsedMs}ms | ${chk.relationToControl?.selectorRelation || '-'} | ${chk.relationToControl?.paretoRelation || '-'} |`)
      }
    }
  }
  ...
  md.push('Arm B imposes only the cost of unpolished filter evaluation (31 extra canonical evaluations). Arm D imposes enormous overhead (thousands of evaluations for unpolished lookahead), drastically inflating canonical evaluations.')
  ...
  md.push('- **C**: Selector is decent but misses explicit bridging necessary for the primary Storm bridge.')
  md.push('- **D**: Lookahead provides signal but is far too expensive to be scalable or justifiable.')
  md.push('\n## Algorithm candidate evaluation')
  md.push(`Arm B meets the criterion for experimental algorithm candidate. It demonstrates robust outcome-blind selection, escaping the plateau on the Storm bridge parent and retaining U12t and Trio performance without extreme overhead. (Eligible: ${report.algorithmCandidateEvaluation.eligible})`)
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) { runCampaign() }
