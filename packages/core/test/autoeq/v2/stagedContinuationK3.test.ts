...
describe('Standard v2 staged continuation experiment K=3', () => {
  it('continues all three staged candidates after the fast pass', () => {
    ...
    // Three staged candidates receive the one-cycle fast pass, then all
    // three receive continuation refinement. No fallback is needed.
    expect(result.jointRefinementCount).toBe(6)
  })
})
