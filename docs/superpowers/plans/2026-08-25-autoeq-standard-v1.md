...
- [ ] **Step 1: Write quantization RED tests** for constrained grid projection on the 1 Hz / 0.1 dB / 0.01 Q grid, including decimal envelopes with no representable point (filter omitted), exactly one representable point, nearest representable selection, deterministic ties, shelf Q 0.7 independent of PK-Q bounds, decimal normalization, and `-0 -> 0`.
- [ ] **Step 2: Run** `pnpm --filter @autoeq-workbench/core test -- test/autoeq/quantize.test.ts` and confirm RED.
- [ ] **Step 3: Implement constrained grid projection** with decimal-safe nearest representable selection. For PK, frequency/gain/Q must each be representable; for shelves, frequency/gain must be representable and Q remains 0.7 independently of PK-Q bounds. Omit a filter when any required coordinate has an empty grid/envelope intersection; do not reject the run.
- [ ] **Step 4: Write discrete-refinement RED tests** proving two passes of current/±1-step search never worsen the raw quantized objective, never leave the policy grid or effective bounds, and operate only on filters that survived projection.
- [ ] **Step 5: Implement discrete refinement** with the same deterministic tie-breakers used by continuous refinement.
- [ ] **Step 6: Write pipeline RED tests** covering source/target preparation on 96 ppo; fit subset inside effective frequency range; one known inverse-PK case reaching MAE <0.25 dB with `maxFilters<=5`; two identical runs deep-equal; all non-representable filters producing a successful empty result; every final coordinate simultaneously satisfying the manual grid and exact effective envelope; final zero-gain filters removed; final list frequency-sorted; IDs exactly `autoeq-1..N`; final audit derived after quantization/cleanup and after final IDs are assigned; final metrics derived from delivered filters over the effective fit interval; preamp derived from delivered filters through `calculatePreampDb()` on the full supported band; manifest contains no timestamp, UUID, or browser runId.
- [ ] **Step 7: Implement exact pipeline order:** validate → prepare canonical grid → select fit subset → desired correction → greedy optimize → prune/final continuous refine → constrained grid projection with non-representable filter omission → discrete refine surviving filters → remove delivered 0 dB filters → deterministic final sort → assign `autoeq-1..N` → exact final cascade → final metrics → full-band preamp → final cancellation audit → manifest. An empty delivered list is a successful result.
