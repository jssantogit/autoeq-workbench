...
**Goal:** Implement the approved deterministic Standard AutoEQ v1 engine, run it in a disposable Web Worker, and integrate successful results atomically into the existing post-remake workspace.

**Architecture:** `packages/core` owns all AutoEQ numerics and reproducible result contracts. The web layer captures immutable run inputs, executes one disposable Worker per run, rejects obsolete results, and applies a successful `AutoEqResult` through one undoable workspace action. Existing Equalizer settings, graph semantics, Tools audio, Compare A/B, and workspace provenance remain authoritative consumers of canonical filters.

**Tech Stack:** TypeScript 6, React 19, Zustand 5, Vite Web Workers, Vitest/jsdom, existing `@autoeq-workbench/core` DSP/curve/metrics APIs.
...
- The spec above is the behavioral authority. This plan is only the execution map.
- Preserve the approved Remake 04 baseline and all source-first graph/layout contracts.
...
- `AutoEqSettings` may only narrow the effective run envelope. Invalid settings are rejected, never silently clamped into validity.
...
apps/web/src/styles/** only where required for observed AutoEQ status/responsive behavior
```
...
export interface RunManifest {
  ...
  sourceName: string
  targetName: string
  algorithmParameters: typeof STANDARD_V1_CONFIG.algorithm
  finalFilters: Filter[]
...
- [ ] **Step 3: Implement `STANDARD_V1_CONFIG` exactly as approved:** deadband `0.1`, Huber delta `1`, candidate threshold `0.5`, minimum objective improvement `0.005`, prune tolerance `0.002`, filter-count weight `0.01`, high-Q weight `0.002`, gain weight `0.0005`, cancellation weight `0.01`. Do not duplicate product hard bounds into this object.
- [ ] **Step 4: Implement `resolveStandardAutoEqConfig()`** using `isValidAutoEqSettings()`. It must reject invalid input rather than clamp/widen it and must copy the valid effective envelope unchanged.
...
- [ ] **Step 6: Implement `evaluateObjective()`** as `mean(deadbandedHuber) + structural penalties`; empty residual input is a validation/numeric error rather than `NaN`.
- [ ] **Step 7: Run** the two new test files plus core typecheck and confirm GREEN.
...
**Interfaces:**
- Consumes: `AutoEqConfig`, fit-grid frequencies, residual dB.
- Produces:
...
- [ ] **Step 1: Write RED tests** for sub-threshold termination, sign-change splitting, geometric-center PK generation, effective gain/Q/Fc clamping, LS evidence, HS evidence, effective-range exclusion, stable ordering, and 1/48-octave same-type deduplication.
- [ ] **Step 2: Confirm RED** with the candidate test file.
- [ ] **Step 3: Implement residual regions** with equal-length/finiteness validation.
- [ ] **Step 4: Implement PK candidate seeds** using geometric center and `Q = center / max(1e-9, end-start)`, clamped to `config.minPkQ..maxPkQ`; initial gain comes from log-frequency interpolation of residual at center and is clamped to effective gain.
- [ ] **Step 5: Implement shelves** only when at least 70% of eligible samples in the approved low/high region share sign and median absolute residual is at least threshold. Keep Q 0.7 and reject a nominal shelf Fc outside the effective run range.
- [ ] **Step 6: Implement deterministic sort/dedupe** exactly from the spec.
...
**Interfaces:**
- Consumes: current filters, desired correction, fit frequencies, `AutoEqConfig`, objective evaluator, candidate generator, existing `cascadeMagnitudeDb()`.
- Produces:
...
- [ ] **Step 1: Write a RED synthetic one-PK refinement test** starting near 1 kHz / +6 dB / Q2 and assert final cascade MAE is lower than initial MAE.
- [ ] **Step 2: Write RED envelope tests** proving no refined Fc/gain/PK-Q escapes the effective config and shelves remain Q 0.7.
- [ ] **Step 3: Implement the three approved coordinate passes:** `1/6 octave, 1 dB, 1/2 Q-octave`; `1/24, 0.25, 1/8`; `1/96, 0.1, 1/32`. Evaluate the complete cascade for every tested coordinate.
...
- [ ] **Step 6: Implement `optimizeGreedy()`** from zero filters. Recompute residual from the entire accepted cascade after each addition. Never mutate input arrays or reuse current workspace filters.
...
- [ ] **Step 4: Write RED pruning tests** for <0.05 dB removal, objective-neutral/removal-tolerant filters removed, required +6 dB synthetic PK retained, restart-from-zero after removal, and final refine pass.
...
export function quantizeFilters(filters: readonly Filter[]): Filter[]
export function discreteRefine(/* typed input from previous tasks */): Filter[]
...
- [ ] **Step 2: Implement quantization** with decimal-safe nearest-step rounding.
- [ ] **Step 3: Write discrete-refinement RED test** proving two passes of current/±1-step search never worsen the raw quantized objective and never leave policy/effective bounds.
- [ ] **Step 4: Implement discrete refinement** with the same deterministic tie-breakers used by continuous refinement.
- [ ] **Step 5: Write pipeline RED tests** covering: source/target preparation on 96 ppo; fit subset inside effective frequency range; one known inverse-PK case reaching MAE <0.25 dB with `maxFilters<=5`; two identical runs deep-equal; final zero-gain filters removed; final list frequency-sorted; IDs exactly `autoeq-1..N`; final audit derived after quantization/cleanup; final metrics derived from delivered filters over the effective fit interval; preamp derived from delivered filters using existing full-band dense-grid policy; manifest contains no timestamp/UUID/runId.
- [ ] **Step 6: Implement pipeline order:** validate → prepare canonical grid → select fit subset → desired correction → greedy optimize → prune/final continuous refine → quantize → discrete refine → remove delivered 0 dB filters → deterministic final sort/IDs → exact final cascade → final metrics → full-band preamp → final cancellation audit → manifest.
- [ ] **Step 7: Confirm `result.filters`, `manifest.finalFilters`, metrics, preamp, and audit describe the same final filter list.**
- [ ] **Step 8: Run full core test/typecheck** and commit `feat(core): complete Standard AutoEQ v1 pipeline`.
...
**Interfaces:**
- Add one non-redundant workspace record:
...
- Extend `FilterSnapshotState`, `WorkspaceHistorySnapshot`, `EqSnapshot`, and `EqSnapshotCapture` with `autoEqRun`.

- [ ] **Step 1: Write workspace RED tests** proving `applyAutoEqResult()` is one undo point, sets delivered filters, clears selection, sets provenance `autoeq`, state `clean`, and stores a deep copy of the manifest.
...
- [ ] **Step 3: Write lifecycle RED tests** proving manual edit after AutoEQ keeps the run record and marks `modified`; active FR/Target, normalization, or settings change keeps the record and marks `stale`; importing a new manual filter set clears the record.
- [ ] **Step 4: Implement `autoEqRun` + `applyAutoEqResult()`**. This action is the only production route that creates a clean AutoEQ solution. Validate/deep-copy result filters before recording history.
- [ ] **Step 5: Extend history snapshots** to deep-copy/restore the run record.
...
- [ ] **Step 7: Extend Compare capture/recorder** with `autoEqRun`; keep snapshot cap/debounce/coalescing semantics from Remake 04 unchanged.
- [ ] **Step 8: Run state tests + web typecheck** and commit `feat(web): track AutoEQ run provenance in workspace history`.
...
export interface AutoEqRunUiState {
  ...
  error: { category: 'validation' | 'optimization' | 'numeric'; message: string } | null
}
...
Worker protocol is serializable and includes a browser-only `runId` on run/result/error messages. `runId` never enters core manifest data.

- [ ] **Step 1: Write client RED tests** with a fake Worker adapter for result, structured error, terminate-on-cancel, fresh worker next run, and late-message/runId rejection.
- [ ] **Step 2: Implement `autoeq.worker.ts`** importing only public core APIs and serializing `CoreError` as category + message.
- [ ] **Step 3: Implement `autoeqClient.ts`** as one disposable worker per run; no persistent worker pool and no fake progress.
- [ ] **Step 4: Write transient-store RED tests** for `idle -> running -> idle`, `idle -> running -> error`, error dismiss/reset, and cancellation returning to idle without an error banner.
- [ ] **Step 5: Implement `autoeqRunStore`** separate from undoable workspace state.
- [ ] **Step 6: Write controller RED tests** proving it captures immutable active FR/Target/normalization/settings, rejects runs when workspace is not ready, applies only valid current results, and never mutates filters on cancel/error.
- [ ] **Step 7: Define a pure run-input signature** from active FR ID + active Target ID + their numerical raw point data + normalization + `AutoEqSettings`. Display names are provenance only and do not affect numerical obsolescence. Removing/replacing selected numerical input must change the signature.
- [ ] **Step 8: Add obsolete-result RED tests** where FR/Target selection, curve numerical data, normalization, or settings change before resolution; confirm result is discarded and prior solution survives. Also test a mere unrelated UI/theme change does not obsolete the result.
- [ ] **Step 9: Implement controller** to preflight current workspace, capture input/signature, start client, compare current signature before commit, call `applyAutoEqResult()` only on match, and always clear the active run on terminal paths.
- [ ] **Step 10: Run focused worker/controller tests + web typecheck** and commit `feat(web): run AutoEQ safely in a disposable worker`.
...
- Modify: existing stylesheet file(s) only for compact status/error styling if tests/manual review demonstrate need.

**Interfaces:**
- `EqualizerTab` may receive the already-computed `WorkspaceDerived` from `App` so readiness is not recomputed in a second UI path.
- Existing `AutoEqConstraints` remains unchanged unless an observed integration defect requires a minimal fix.

- [ ] **Step 1: Write UI RED tests** proving AutoEQ is disabled when there is no valid active FR+Target/coverage, enabled when ready and idle, and no duplicate Standard settings surface appears.
- [ ] **Step 2: Write running-state RED test** proving the main action becomes `Cancel` while running, there is no percentage/progress meter, and clicking it invokes `cancelAutoEq()`.
- [ ] **Step 3: Write error-state RED test** proving a structured public error is shown compactly with accessible alert semantics while existing filters remain visible.
- [ ] **Step 4: Write success integration RED test** with mocked controller result proving the canonical filter table updates, solution becomes clean AutoEQ, and graph/Tools consumers update through existing canonical state rather than result-specific side channels.
- [ ] **Step 5: Implement UI wiring** into the existing AutoEQ button/button row. Keep Constraints, Filter I/O, FR selector, and Target selector in their current source-shaped composition.
- [ ] **Step 6: Pass `derived` from `App` to `EqualizerTab`** if required for readiness. Do not add AutoEQ recomputation or DSP to React.
- [ ] **Step 7: Run Equalizer tests, graph tests, Tools tests, web typecheck/build** and commit `feat(web): expose Standard AutoEQ workflow`.
...
**Files:**
- Modify only defects observed during this gate.
- Do not tune Standard algorithm constants here.
...
- [ ] **Step 2: Audit architecture boundaries:** no React/Zustand/Worker imports in `packages/core`; no optimization formulas duplicated in web; no runtime imports from `vendor/**`; no Same-Rig/Cross-Rig/Consensus/weighting implementation in Standard v1.
- [ ] **Step 3: Run deterministic core smoke** twice on the same synthetic known-PK case and record deep equality + MAE <0.25 dB + `<=5` filters.
- [ ] **Step 4: Run settings edge smoke:** defaults, narrowed valid envelope, invalid envelope rejection, `maxFilters=0`, `maxFilters=64` without fill-target behavior.
- [ ] **Step 5: Run workspace smoke:** existing manual filters → Run → clean AutoEQ → manual edit → modified → Undo → clean AutoEQ → normalization/settings/input change → stale → Undo/Redo restores matching run record.
- [ ] **Step 6: Run Compare A/B smoke:** capture two solutions, assign A/B, switch A→B→A, verify filters + provenance + state + AutoEQ run record remain paired and deterministic.
- [ ] **Step 7: Run Tools audio smoke in a real browser:** after AutoEQ success, Tone and local Music with `EQ Effect` audibly use the new canonical filter cascade without a second audio/EQ result path.
...
- [ ] **Step 10: Push coherent commits and wait for CI.** Record final implementation SHA and confirm CI is green on that exact SHA.
- [ ] **Step 11: Confirm Pages workflow checks out/builds that exact implementation SHA** before deployment. Keep the known `workflow_run` administrative `pages_build_version` nuance distinct from the actual checked-out content SHA.
...
- identical input/settings produce deeply identical delivered result and manifest;
- final values lie on the delivery quantization grid and inside the effective run envelope;
- `maxFilters` remains a ceiling and additional low-value filters stop below it;
...
- success is exactly one undoable workspace replacement with `filterProvenance='autoeq'`, `solutionState='clean'`, and matching run record;
